...
아래 그림은 개선된 Traffic Signal Controller의 최종 state diagram입니다.

![State Diagram](../images/state_diagram.png)

기존 Traffic Signal Controller의 일반 동작 흐름은 `S0 → S1 → S2 → S3 → S4 → S0`입니다.

여기에 기능 개선을 위해 두 개의 state를 추가했습니다.

* `S5`: Emergency Mode
* `S6`: Night OFF Mode

`emergency = 1`이 입력되면 어떤 일반 state에 있더라도 `S5`로 이동하도록 설계했습니다.
또한 Night Mode 중 emergency가 발생하면 `S6 → S5`로 이동하고, emergency가 종료되었을 때 `night_mode = 1`이 유지 중이면 다시 `S6`로 복귀합니다.

**Summary:**
The state diagram extends the original traffic signal controller by adding `S5` for Emergency Mode and `S6` for Night OFF Mode.

---

## 3. State Definition and Output Table

아래 표는 각 state의 encoding과 출력 상태를 정리한 표입니다.

![State Output Table](../images/state_output_table.png)
...
### 6.1 S0: Main Highway Green / Country Road Red
...
### 6.2 S1: Main Highway Yellow / Country Road Red
...
### 6.3 S2: Main Highway Red / Country Road Red
...
### 6.4 S3: Main Highway Red / Country Road Green
...
### 6.5 S4: Main Highway Red / Country Road Yellow
...
## 7. Priority of Input Conditions
